...
public class TextMapParser implements IParser {
  ...
  public TextMapParser( final byte[] buffer , final int start , final int length , final MapContainerField schema ) throws IOException{
    this.buffer = buffer;
    this.start = start;
    this.length = length;
  ...
  private PrimitiveObject parse( final byte target ) throws IOException{
    ...
    PrimitiveObject retVal = new BytesObj( buffer , readOffset , ( length - ( readOffset - start ) ) );
